...
class AdsorptionCurveProcessor:
    ...
    def create_warning_visualization(self, efficiency_data: pd.DataFrame, base_datetime=None) -> plt.Figure:
        ...
        # 横坐标拉伸系数（初始值为1.0，表示不拉伸）
        stretch_factor = 1.0

        # 计算累积时间段时长（支持拉伸的新时间计算方式）
        def calculate_cumulative_segment_time(data, stretch_factor=1.0):
            """计算累积的时间段时长，支持横坐标拉伸"""
            cumulative_hours = []
            total_duration = 0
            ...
            for i in range(len(data)):
                ...
                # 应用拉伸系数
                stretched_duration = segment_duration * stretch_factor
                total_duration += stretched_duration
                cumulative_hours.append(total_duration / 3600)  # 转换为小时

            return cumulative_hours

        # 使用新的时间计算方式（初始不拉伸）
        time_hours = calculate_cumulative_segment_time(efficiency_data, stretch_factor)
        ...
        def on_click(event):
            ...
            if event.inaxes == ax1 and event.button == 1:  # 左键点击
                # 获取当前拉伸状态下的时间坐标
                nonlocal stretch_factor
                current_time_hours = calculate_cumulative_segment_time(efficiency_data, stretch_factor)

                # 检查鼠标是否在散点附近
                min_distance = float('inf')
                closest_index = -1

                for i, (time_h, bt_pct) in enumerate(zip(current_time_hours, breakthrough_percent)):
                    # 计算鼠标位置与数据点的距离
                ...
                if closest_index >= 0 and min_distance < 2.0:  # 增加点击容忍度
                    ...
                    # 计算原始累积时长（不拉伸）
                    original_cumulative_hours = calculate_cumulative_segment_time(efficiency_data, 1.0)[i]

                    # 创建标注文本（模仿清洗后图像的格式，并显示累积时间和拉伸信息）
                    if stretch_factor != 1.0:
                        annotation_text = f"时间段: {time_segment}\n累积时长: {cumulative_hours:.2f}小时 (拉伸后)\n原始时长: {original_cumulative_hours:.2f}小时\n拉伸系数: {stretch_factor:.2f}\n穿透率: {bt_pct:.1f}%"
                    else:
        ...
        def on_scroll(event):
            ...
            print(f"滚轮事件触发: {event.button}, 当前拉伸系数: {stretch_factor:.3f}")  # 调试信息

            # 滚轮向上增加拉伸，向下减少拉伸
            old_stretch_factor = stretch_factor
            if event.button == 'up':
                stretch_factor *= 1.2  # 增加20%，更明显的变化
            elif event.button == 'down':
                stretch_factor /= 1.2  # 减少20%，更明显的变化

            # 限制拉伸系数范围 (0.1 到 10.0)
            stretch_factor = max(0.1, min(10.0, stretch_factor))

            # 如果拉伸系数没有变化，直接返回
            if abs(stretch_factor - old_stretch_factor) < 0.001:
                print("拉伸系数未变化，跳过重绘")
                return

            print(f"新拉伸系数: {stretch_factor:.3f}")  # 调试信息

            # 重新计算拉伸后的时间坐标
            new_time_hours = calculate_cumulative_segment_time(efficiency_data, stretch_factor)

            print(f"原始时间范围: {time_hours[0]:.2f} - {time_hours[-1]:.2f}")  # 调试信息
            print(f"拉伸后时间范围: {new_time_hours[0]:.2f} - {new_time_hours[-1]:.2f}")  # 调试信息
            print(f"数据点数量: {len(new_time_hours)}")  # 调试信息
            ...
            # 重新绘制主要穿透曲线（使用新的时间坐标）
            ax1.plot(new_time_hours, breakthrough_percent, 'b-', linewidth=1.5,
                    alpha=0.8, label='实际穿透率(出口浓度/进口浓度)')
            print(f"重绘曲线完成，数据点: {len(new_time_hours)}")  # 调试信息

            # 重新绘制数据点（使用新的时间坐标）
            ax1.scatter(new_time_hours, breakthrough_percent,
                       color='blue', s=15, alpha=0.4, zorder=3,
                       edgecolors='darkblue', linewidth=0.5,
                       label='时间段穿透率数据点')
            print("重绘散点完成")  # 调试信息
            ...
            # 重新绘制关键数据点和标签
            for i, (time_h, bt_pct) in enumerate(zip(new_time_hours, breakthrough_percent)):
                if i % max(1, len(new_time_hours)//15) == 0:
                    ax1.scatter(time_h, bt_pct, color='red', s=80, zorder=5,
                               edgecolors='darkred', linewidth=1)
                    ax1.annotate(f'{bt_pct:.1f}%',
                               xy=(time_h, bt_pct),
            ...
            if self.warning_model.fitted:
                ...
                for t in time_smooth:
                    num_segments = (t - efficiency_data['time'].min()) / avg_interval
                    cumulative_time = num_segments * avg_interval * stretch_factor / 3600
                    cumulative_smooth_hours.append(cumulative_time)
                ...
                # 重新绘制关键时间点（拉伸后）
                def convert_to_cumulative_time_stretched(original_time):
                    if original_time <= efficiency_data['time'].min():
                        return 0
                    avg_interval = np.mean(np.diff(efficiency_data['time']))
                    num_segments = (original_time - efficiency_data['time'].min()) / avg_interval
                    return num_segments * avg_interval * stretch_factor / 3600

                if self.warning_model.breakthrough_start_time:
                    start_time_h = convert_to_cumulative_time_stretched(self.warning_model.breakthrough_start_time)
                    ax1.axvline(x=start_time_h, color='green', linestyle=':', alpha=0.8,
                               label='穿透起始时间')

                if self.warning_model.warning_time:
                    warning_time_h = convert_to_cumulative_time_stretched(self.warning_model.warning_time)
                    ax1.axvline(x=warning_time_h, color='orange', linestyle=':', alpha=0.8,
                               label='预警时间点(80%)')
                ...
                if self.warning_model.predicted_saturation_time:
                    sat_time_h = convert_to_cumulative_time_stretched(self.warning_model.predicted_saturation_time)
                    ax1.axvline(x=sat_time_h, color='red', linestyle=':', alpha=0.8,
                               label='实际饱和时间')

            # 重新设置图表属性
            ax1.set_xlabel(f'累积运行时间 (小时) - 拉伸系数: {stretch_factor:.2f}', fontsize=12)
            ax1.set_ylabel('穿透率 (%)', fontsize=12)
            ax1.set_title('穿透率变化趋势与预警分析 (鼠标滚轮调整拉伸)', fontsize=14, fontweight='bold')
            ax1.legend(loc='upper left')
            ax1.grid(True, alpha=0.3)
            ax1.set_ylim(0, 105)
        ...
        # 添加使用说明到图表标题下方
        fig.text(0.5, 0.95,
                '使用说明: 在主图区域使用鼠标滚轮可调整横坐标拉伸 (向上滚动拉伸，向下滚动压缩)',
