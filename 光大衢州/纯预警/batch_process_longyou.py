...
class BatchProcessorGuangdaQuzhou:
    """光大衢州批量处理器"""

    def __init__(self):
        self.warning_system = WasteIncinerationWarningSystemGuangdaQuzhou()
        self.all_warnings = []
    ...
    def process_single_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            print(f"  - 处理完成，发现 {len(warnings_list)} 条预警")
            return True
    ...
    def save_results(self, output_file: str = "光大衢州_批量预警结果.xlsx"):
        """保存处理结果"""
        try:
            ...
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                # 预警结果
                df_warnings.to_excel(writer, sheet_name='预警结果', index=False)
            ...
            print(f"发现 {len(self.all_warnings)} 条预警")
            if self.failed_files:
...
def main():
    ...
    try:
        ...
        # 保存结果
        output_filename = f"光大衢州_批量预警结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
