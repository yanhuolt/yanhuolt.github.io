...
LONGYOU_FIELD_MAPPING = {
    # 炉膛温度相关字段 (三个炉子) - 根据新规则中的数据采集点位
    # 注意：规则中提到的U1、V1、W1等点位在实际CSV中对应为炉膛上部和中部温度
    "furnace_1_top_temp": "1#炉膛上部温度",  # 对应规则中的U1、V1、W1
    "furnace_1_mid_temp": "1#炉膛中部温度",  # 对应规则中的X1、Y1、Z1
    "furnace_2_top_temp": "2#炉膛上部温度",  # 对应规则中的AA1、AB1、AC1
    "furnace_2_mid_temp": "2#炉膛中部温度",  # 对应规则中的AD1、AE1、AF1
    ...
    # 污染物浓度 (三个炉子) - 根据新规则中的数据采集点位
    # 1号炉 - 对应规则中的ES、EU、ET、EW、EV
    "furnace_1_dust": "1#炉烟气KLW（折算）",    # ES - 颗粒物
    "furnace_1_nox": "1#炉烟气NOX（折算）",     # EU - 氮氧化物
    "furnace_1_so2": "1#炉烟气SO2（折算）",     # ET - 二氧化硫
    "furnace_1_hcl": "1#炉烟气HCL（折算）",     # EW - 氯化氢
    "furnace_1_co": "1#炉烟气CO（折算）",       # EV - 一氧化碳

    # 2号炉 - 对应规则中的FD、FF、FE、FH、FG
    "furnace_2_dust": "2#炉烟气KLW（折算）",    # FD - 颗粒物
    "furnace_2_nox": "2#炉烟气NOX（折算）",     # FF - 氮氧化物
    "furnace_2_so2": "2#炉烟气SO2（折算）",     # FE - 二氧化硫
    "furnace_2_hcl": "2#炉烟气HCL（折算）",     # FH - 氯化氢
    "furnace_2_co": "2#炉烟气CO（折算）",       # FG - 一氧化碳
...
LONGYOU_WARNING_THRESHOLDS = {
    # 1. 低炉温焚烧报警 (5分钟平均值)
    "low_furnace_temp": 850,  # 低于850℃触发报警

    # 2-6. 污染物日均值排放超标报警阈值
    "dust_daily_limit": 20,   # 颗粒物（PM）日均值 ≤20mg/m³
    "nox_daily_limit": 250,   # 氮氧化物（NOx）日均值 ≤250mg/m³
    "so2_daily_limit": 80,    # 二氧化硫（SO₂）日均值 ≤80mg/m³
    "hcl_daily_limit": 50,    # 氯化氢（HCl）日均值 ≤50mg/m³
    "co_daily_limit": 80,     # 一氧化碳（CO）日均值 ≤80mg/m³
            ...
            for pollutant, (event_name, threshold_key) in pollutants.items():
                ...
                if field and field in df_daily.columns:
                    ...
                    for _, row in df_daily[exceed_mask].iterrows():
                        # 根据规则确定是预警还是报警
                        alarm_type = '报警' if pollutant in ['so2', 'co'] else '预警'
