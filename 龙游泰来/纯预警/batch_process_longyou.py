...
class BatchProcessorLongyou:
    """龙游泰来批量处理器"""
    
    def __init__(self):
        self.warning_system = WasteIncinerationWarningSystemLongyou()
        self.all_warnings = []
    ...
    def save_results(self, output_file: str = "龙游泰来_批量预警结果.xlsx"):
        """保存处理结果"""
        try:
...
def main():
    ...
    try:
        ...
        # 保存结果
        output_filename = f"龙游泰来_批量预警结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
