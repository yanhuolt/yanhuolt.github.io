...
class AdsorptionCurveProcessor:
        ...
        while i < len(all_segment_ids):
            ...
            if current_type == 1:  # 当前是进口时间段
                ...
                if next_outlet_segment_id is not None:
                    # 匹配成功，根据需求文档第21行：将两段时间拼接然后进行筛选
                    inlet_data = data[data['时间段序号'] == current_segment_id]
                    outlet_data = data[data['时间段序号'] == next_outlet_segment_id]

                    # 将两段时间拼接为一个大的时间段
                    combined_data = pd.concat([inlet_data, outlet_data], ignore_index=True)
                    combined_data = combined_data.sort_values('创建时间').reset_index(drop=True)

                    # 添加拼接时间段标记
                    combined_segment_id = f"{current_segment_id}_{next_outlet_segment_id}"
                    combined_data['拼接时间段'] = combined_segment_id
                    ...
                    if len(valid_outlet_data) > 0:
                        ...
                        inlet_data_copy['拼接时间段'] = f"{current_segment_id}_only"
                        valid_records.append(inlet_data_copy)
                    ...
                    print(f"        拼接为大时间段: {combined_segment_id}")

                    # 跳过已处理的出口时间段
                    i = j + 1
                else:
                    # 根据需求文档第19行：如果没有匹配到下一个对应的进口浓度或出口浓度时间段，请弃用该段时间
                    print(f"      时间段 {current_segment_id}(进口) 没有匹配的出口时间段，弃用该段时间")
                    i += 1
                ...
                if next_inlet_segment_id is not None:
                    ...
                    # 将两段时间拼接为一个大的时间段
                    combined_data = pd.concat([outlet_data, inlet_data], ignore_index=True)
                    combined_data = combined_data.sort_values('创建时间').reset_index(drop=True)

                    # 添加拼接时间段标记
                    combined_segment_id = f"{current_segment_id}_{next_inlet_segment_id}"
                    combined_data['拼接时间段'] = combined_segment_id
                    ...
                    if len(valid_outlet_data) > 0:
                        ...
                        # 如果出口数据全部被筛选掉，只保留进口数据
                        inlet_data_copy = inlet_data.copy()
                        inlet_data_copy['拼接时间段'] = f"{next_inlet_segment_id}_only"
                        valid_records.append(inlet_data_copy)
                    ...
                    # 根据需求文档第19行：如果没有匹配到下一个对应的进口浓度或出口浓度时间段，请弃用该段时间
                    print(f"      时间段 {current_segment_id}(出口) 没有匹配的进口时间段，弃用该段时间")
                    i += 1
            else:
                i += 1
    ...
    def _calculate_efficiency_for_switching(self, data: pd.DataFrame, method_name: str) -> Optional[pd.DataFrame]:
        """为切换型数据计算效率 - 根据需求文档第21行：将两段时间拼接为一个大的时间段计算穿透率"""
        print(f"切换型数据效率计算（拼接时间段计算穿透率）")
        ...
        for wind_segment_id in sorted(wind_segments):
            ...
            # 根据需求文档：匹配进口和出口时间段，将两段时间拼接为一个大的时间段
            i = 0
            while i < len(segment_ids):
                ...
                if current_type == 1:  # 进口时间段
                    ...
                    if next_outlet_segment_id is not None:
                        ...
                        # 根据需求文档第19行：删除明显异常的穿透率数据点
                        if avg_inlet > 0:
                            breakthrough_ratio = avg_outlet / avg_inlet
                            # 检查穿透率是否异常（>1.0为明显异常）
                            if breakthrough_ratio > 1.0:
                                print(f"      检测到异常穿透率 {breakthrough_ratio:.3f} (>1.0)，删除该数据点")
                                i = j + 1
                                continue
                        ...
                        # 计算拼接时间段的时间坐标（整个拼接段的中间时间）
                        combined_start = combined_data['创建时间'].min()
                        combined_end = combined_data['创建时间'].max()
                        ...
                        efficiency_records.append({
                            ...
                            'combined_segment': f"{current_segment_id}_{next_outlet_segment_id}"
                        })

                        print(f"风速段{wind_segment_id}-拼接段{current_segment_id}+{next_outlet_segment_id}: 进口={avg_inlet:.2f}, 出口={avg_outlet:.2f}, 穿透率={breakthrough_ratio:.3f}, 效率={efficiency:.1f}%")

                        # 跳过已处理的出口时间段
                        i = j + 1
                    else:
                        # 根据需求文档第19行：如果没有匹配到，弃用该段时间
                        print(f"      进口时间段{current_segment_id}没有匹配的出口时间段，弃用")
                        i += 1
                    ...
                    if next_inlet_segment_id is not None:
                        ...
                        efficiency_records.append({
                            ...
                            'combined_segment': f"{current_segment_id}_{next_inlet_segment_id}"
                        })

                        print(f"风速段{wind_segment_id}-拼接段{current_segment_id}+{next_inlet_segment_id}: 进口={avg_inlet:.2f}, 出口={avg_outlet:.2f}, 穿透率={breakthrough_ratio:.3f}, 效率={efficiency:.1f}%")

                        # 跳过已处理的进口时间段
                        i = j + 1
                    else:
                        # 根据需求文档第19行：如果没有匹配到，弃用该段时间
                        print(f"      出口时间段{current_segment_id}没有匹配的进口时间段，弃用")
                        i += 1
                else:
                    i += 1
        ...
        if efficiency_records:
            efficiency_df = pd.DataFrame(efficiency_records)
            print(f"生成效率数据: {len(efficiency_df)} 个拼接时间段")
            print(f"平均效率: {efficiency_df['efficiency'].mean():.2f}%")
            print(f"平均穿透率: {efficiency_df['breakthrough_ratio'].mean():.3f}")
            return efficiency_df
        else:
